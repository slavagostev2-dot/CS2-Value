    ...
    *,
    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            # Some builds expose the finished state as a client-side tab even when /finished
            # was requested. Click it only if present; otherwise keep the requested route.
            try:
            ...
            stable_rounds = 0
            best: list[str] = []
            # The listing currently grows in relatively small client-side batches.
            # For a deep-history target (e.g. 3000), the old fixed 40 rounds could
            # stop long before the requested count. Scale the ceiling with the target
            # while keeping a hard cap so a changed page cannot loop forever.
            ...
            for _ in range(effective_rounds):
                ...
                if clicked:
                    # Give the client time to append a new batch before measuring again.
                    page.wait_for_timeout(1_200)
                else:
                    # Trigger both ordinary scroll handlers and intersection observers.
                    try:
                ...
                # A single unchanged round is not enough: client-side fetches can be delayed.
    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            for round_no in range(max(1, max_rounds)):
                ...
                # Current match cards can appear after a delayed client-side fetch.
